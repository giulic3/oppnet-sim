...
for (s in 1:length(seeds)) {
  for (it in 1:length(iterationVars)) {
    for (j in 0:(runs-1)) {
        q1Length <-read.csv(paste("General-",seeds[s], ",",iterationVars[it],"-#",j,"_q1length.csv", sep=""), header=TRUE, sep=',')
        #cat(paste("General-",iterationVars[i],"-#",j,"_q1length.csv", sep=""), '\n')
        q2Length <-read.csv(paste("General-",seeds[s], ",",iterationVars[it],"-#",j,"_q2length.csv", sep=""), header=TRUE, sep=',')
        q3Length <-read.csv(paste("General-",seeds[s], ",",iterationVars[it],"-#",j,"_q3length.csv", sep=""), header=TRUE, sep=',')
        lifeTime <-read.csv(paste("General-",seeds[s], ",",iterationVars[it],"-#",j,"_lifetime.csv", sep=""), header=TRUE, sep=',')
...
#        cat('mean throughput = ', meanThroughput, '\n')
        ...
        results_column <- c(paste("General-",seeds[s], ",",iterationVars[it],"-#",j, sep=""))
        for (i in 1:4)
